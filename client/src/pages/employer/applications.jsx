    ...
    <>
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="mb-6 flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-white">{t('applications.title')}</h1>
        </div>

        <div className="bg-slate-900/60 backdrop-blur-lg border border-purple-500/20 rounded-xl p-6">
          <div className="mb-4">
            <label className="text-sm text-gray-300 mr-2">{t('applications.filterByJob')}</label>
          ...
          {loading ? (
            <div className="text-purple-300">{t('messages.loading')}</div>
          ) : applications.length === 0 ? (
            <div className="text-gray-300">{t('applications.noApplicationsYet')}</div>
          ) : (
            <div className="space-y-4">
              {applications.map(app => (
                <div key={app._id} className="bg-slate-800 rounded-lg p-4 flex flex-col md:flex-row md:justify-between">
                  <div>
                    <div className="text-white font-semibold">{app.jobId?.title || t('jobCard.general')}</div>
                    <div className="text-gray-400 text-sm">{t('applications.applicant')}: {app.jobSeekerId?.name || t('jobCard.general')} ({app.jobSeekerId?.email || ''})</div>
                    {app.coverLetter && <div className="mt-2 text-gray-300 whitespace-pre-line">{app.coverLetter}</div>}
                  </div>

                  <div className="mt-3 md:mt-0 text-right">
                    <div className="text-sm text-gray-400">{t('applications.applied')}: {new Date(app.appliedDate).toLocaleString()}</div>
                    ...
                    <div className="flex items-center justify-end gap-2 mt-2">
                      {app.resume && (
                        (() => {
                          ...
                          return (
                            <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 px-3 py-1 bg-blue-600 text-white rounded">{t('applications.viewResume')}</a>
                          );
                        })()
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-900 rounded-lg p-6 w-full max-w-3xl mx-4">
            <div className="flex justify-between items-start">
              <h2 className="text-2xl font-bold text-white">{selectedProfile.name || t('profile.seekerTitle')}</h2>
              <button onClick={() => { setProfileModalOpen(false); setSelectedProfile(null); }} className="text-gray-300">{t('common.close')}</button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
              <div className="col-span-1 flex flex-col items-center">
                ...
                {selectedProfile.resume && (
                  (() => {
                    ...
                    return (
                      <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="mt-3 px-4 py-2 bg-blue-600 text-white rounded">{t('applications.downloadResume')}</a>
                    );
                  })()
                )}
              </div>
              ...
              <div className="col-span-2">
                <div className="text-gray-300"><strong>{t('profile.contact')}:</strong> {selectedProfile.userId?.phone || selectedProfile.phone || t('jobDetails.notSpecified')}</div>
                <div className="text-gray-300"><strong>{t('login.email')}:</strong> {selectedProfile.email || selectedProfile.userId?.email || t('jobDetails.notSpecified')}</div>
                <div className="text-gray-300"><strong>{t('profile.location')}:</strong> {selectedProfile.address || t('jobDetails.notSpecified')}</div>
                <div className="text-gray-300"><strong>{t('register.role')}:</strong> {selectedProfile.age || t('jobDetails.notSpecified')}</div>
                <div className="mt-3 text-gray-300"><strong>{t('profile.bio')}:</strong>
                  <p className="mt-1 whitespace-pre-line">{selectedProfile.about || t('jobDetails.notSpecified')}</p>
                </div>

                {selectedProfile.skills?.length > 0 && (
                  <div className="mt-3 text-gray-300"><strong>{t('profile.skills')}:</strong> {selectedProfile.skills.join(', ')}</div>
                )}
